        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            foreach (var size in Size.All)
            {
                ...
                var expectedSizeCount = shirts.Count(
                    s => s.Size.Id == size.Id
                         && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Select(optionSize => optionSize.Id)
                                 .Contains(s.Size.Id))
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = shirts.Count(
                    c => c.Color.Id == color.Id
                         && (!searchOptions.Colors.Any() || searchOptions.Colors.Select(optionColor => optionColor.Id)
                                 .Contains(c.Color.Id))
